    ...
    override func spec() {
        ...
        describe("-collectionView:cellForItemAtIndexPath:") {
            ...
            it("returns a NotificationCell") {
                ...
                let toggleCellItem = StreamCellItem(jsonable: notification, type: .Notification, data: nil, oneColumnCellHeight: 60.0, multiColumnCellHeight: StreamCreateCommentCell.Size.Height, isFullWidth: true)
                subject.appendUnsizedCellItems([toggleCellItem], withWidth: webView.frame.width) { cellCount in
                    vc.collectionView.dataSource = subject
                    vc.collectionView.reloadData()
                }
            ...
            it("returns a StreamImageCell") {
                ...
                subject.removeAllCellItems()
                let toggleCellItem = StreamCellItem(jsonable: post, type: .Image, data: imageRegion, oneColumnCellHeight: 60.0, multiColumnCellHeight: StreamCreateCommentCell.Size.Height, isFullWidth: true)
                subject.appendUnsizedCellItems([toggleCellItem], withWidth: webView.frame.width) { cellCount in
                    vc.collectionView.dataSource = subject
                    vc.collectionView.reloadData()
                }
